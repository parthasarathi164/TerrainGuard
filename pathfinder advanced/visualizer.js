...
export const TerrainVisualizer = {
    ...
    drawAxisNumbers() {
        ...
        if (DATA.bounds) {
            // X-AXIS: LONGITUDE (West to East)
            for (let x = 0; x <= CONFIG.GRID_RES; x += stepVal) {
                const ratio = x / CONFIG.GRID_RES;
                const lng = DATA.bounds.minLng + ratio * (DATA.bounds.maxLng - DATA.bounds.minLng);
                const labelText = this.formatCoord(lng, false);
                const sprite = this.createLabelSprite(labelText, '#ffaa00', '20px');
                sprite.position.set(minPos + ratio * CONFIG.PLANE_SIZE, -6.5, minPos - 4);
                axisLabelsGroup.add(sprite);
            }

            // Z-AXIS: LATITUDE (North to South)
            for (let z = 0; z <= CONFIG.GRID_RES; z += stepVal) {
                const ratio = z / CONFIG.GRID_RES;
                const lat = DATA.bounds.maxLat - ratio * (DATA.bounds.maxLat - DATA.bounds.minLat);
                const labelText = this.formatCoord(lat, true);
                const sprite = this.createLabelSprite(labelText, '#00d2ff', '20px');
                sprite.position.set(minPos - 5, -6.5, minPos + ratio * CONFIG.PLANE_SIZE);
                axisLabelsGroup.add(sprite);
            }
        }
        ...
        for (let i = 0; i <= 8; i++) {
            ...
            const sprite = this.createLabelSprite(Math.round(realMeters) + 'm', '#00ff00', '22px');
